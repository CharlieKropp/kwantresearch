...
def Aharonov_Bohm(site1, site2, phi):
    return np.exp(-phi * complex(0,1) * (2 * np.pi) / (W1-1))
#------------------------------------------------------------------------------
...
for i in range(L):
    for j in range(W1):
        if j > 0:
            sys0[lat(i, j), lat(i, j - 1)] = Aharonov_Bohm           
...
for j in range(W1):
    if j > 0:
        left_lead[lat(0, j), lat(0, j - 1)] = Aharonov_Bohm
...
for j in range(W1):
    if j > 0:
        right_lead[lat(0, j), lat(0, j - 1)] = Aharonov_Bohm
...
for rlz in range(nrlz):
    ...
    if wfyn == 'y':
        p = dict(k_x=0,k_y=0,seed=str(rlz), phi = 0.23)
        s = kwant.smatrix(sys,E,params=p,check_hermiticity=True)
    ...
    if kxplot == 'y':
        ...
        pyplot.show()
if tau_mat == 'y':
    sio.savemat('tau.mat', {'taus':taus, 'nrlz':nrlz, 'steps':steps,'nL':nL})
if cvsb == 'y':
    sio.savemat('cvsb.mat', {'allcvsb':allcvsb})