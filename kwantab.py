...
def Aharonov_Bohm_up(site1, site2):
    return t * np.exp((2 * np.pi) / W1)

def Aharonov_Bohm_down(site1, site2):
    return -1 * t * np.exp((2 * np.pi) / W1)

hops = kwant.builder.HoppingKind((0, 1), lat)
sys0[hops(sys0)] = Aharonov_Bohm_up

hops2 = kwant.builder.HoppingKind((0, -1), lat)
sys0[hops2(sys0)] = Aharonov_Bohm_down
#Leads
...
for rlz in range(nrlz):
    ...
    if wfyn == 'y':
        ...
        #----------------------------------------------------------------------
        sio.savemat('wf.mat', {'wf0':wf0,'wfe':wfe,'tp':tp,'te':te})
        print( 'Calculation is done, g=',s.transmission(1,0))
        plot_conductance(sys, energies=[.01 * i for i in range(200)])
